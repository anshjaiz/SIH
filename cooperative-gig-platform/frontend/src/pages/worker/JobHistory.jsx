        ...
        <div className="space-y-4">
          {data.jobs.map((job) => (
            <div key={job._id} className="card">
              ...
              {FAILED_REASON[job.status] && (
                <div className={`mt-3 p-3 rounded-lg text-sm ${
                  job.status === 'WORKER_NO_SHOW' ? 'bg-red-50 text-red-700'
                  : job.status === 'CANCELLED' ? 'bg-gray-100 text-gray-600'
                  : 'bg-yellow-50 text-yellow-700'
                }`}>
                  <span className="font-medium">{FAILED_REASON[job.status]}</span>
                  {job.noShowDetectedAt && (
                    <span className="block text-xs mt-1 opacity-80">
                      Detected: {new Date(job.noShowDetectedAt).toLocaleString()}
                    </span>
                  )}
                  {job.cancellationReason && (
                    <span className="block text-xs mt-1 opacity-80">Reason: {job.cancellationReason}</span>
                  )}
                </div>
              )}
